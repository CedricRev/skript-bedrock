...
@Name(value="Forms - StepSlider")
@Description(value={"Create stepslider on custom form "})
@Examples(value={"form-step-slider named \"step slider\" with elements \"option 1\",\"option 2\",\"option 3\""})
@RequiredPlugins(value={"Floodgate"})
@Since(value="1.0")
public class EffStepSlider
        extends Effect {
    int pattern;
    ...
    public boolean init(Expression<?>[] exprs, int matchedPattern, Kleenean isDelayed, SkriptParser.ParseResult parseResult) {
        ...
        if (!(this.getParser().isCurrentSection(SecCreateCustomForm.class) || (skriptEvent = this.getParser().getCurrentSkriptEvent()) instanceof SectionSkriptEvent && ((SectionSkriptEvent)skriptEvent).isSection(SecFormResult.class))) {
            Skript.error((String)"You can't make a step slider outside of a Custom form creation section.", (ErrorQuality)ErrorQuality.SEMANTIC_ERROR);
            return false;
        }
    ...
    protected void execute(Event event) {
        ...
        switch (this.pattern) {
            case 0: {
                ((CustomForm.Builder)form.getForm()).stepSlider((String)this.name.getSingle(event), (String[])this.elements.getArray(event));
                break;
            }
            case 1: {
                ...
                ((CustomForm.Builder)form.getForm()).stepSlider((String)this.name.getSingle(event), --def, el);
            }
        }
    ...
    static {
        Skript.registerEffect(EffStepSlider.class, (String[])new String[]{"form(-| )(text|step)[(-| )]slider (with name|named) %string% (with|and) [elements] %strings%", "form(-| )(text|step)[(-| )]slider (with name|named) %string% (with|and) [elements] %strings%(, | (with|and) ) [def[ault] [(element [index]|index)]] %number%"});
    }
}
